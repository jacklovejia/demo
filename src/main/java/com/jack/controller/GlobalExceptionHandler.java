...
//        return new Json(eName, false, Codes.SHIRO_ERR, "鉴权或授权过程出错", null);
        return Result.build(Codes.SHIRO_ERR, "鉴权或授权过程出错");
    }
...
//                    .data("detail",e.getMessage());
            return Result.build(Codes.UNAUTHEN, "只允许游客访问，若您已登录，请先退出登录");
        } else {
//            return new Json("Unauthenticated", false, Codes.UNAUTHEN, "用户未登录", null)
//                    .data("detail",e.getMessage());
            return Result.build(Codes.UNAUTHEN, "用户未登录");
        }
    }
...
//        return new Json("Unauthorized", false, Codes.UNAUTHZ, "用户没有访问权限", null);
        return Result.build(Codes.UNAUTHZ, "用户没有访问权限");
    }
